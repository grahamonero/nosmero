    ...
    for (const noteDiv of embeddedNotes) {
        ...
        try {
            ...
            if (event) {
                ...
                // as a note inline; kind 30023 renders as a NIP-23 article
                // preview card; everything else falls back to the NIP-89
                // unknown-kind card with "Open in [client]" deep-links.
                ...
                if (NOTE_LIKE_KINDS.has(event.kind)) {
                    ...
                    try {
                        ...
                        noteDiv.innerHTML = NIP89.renderUnknownKindCard(event, naddr || null);
                        // Hydrate handler links in the background — non-blocking
                        NIP89.hydrateUnknownKindCards(noteDiv).catch(() => {});
                    } catch (e) {
                        // Module failed to load — fall back to existing renderer
