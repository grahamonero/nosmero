...
export function parseContent(content, skipEmbeddedNotes = false) {
    ...
    // .pdf NOT in exclusion: only Nosmero-IPFS PDFs are caught by pdfRegex
    // above; third-party PDFs flow through here as plain links.
        ...
        try {
            ...
            if (event) {
                ...
                if (NOTE_LIKE_KINDS.has(event.kind)) {
                    ...
                    // if the file is on the Nosmero IPFS gateway. Third-party
                    // PDFs fall through to NIP-89 unknown-kind fallback.
