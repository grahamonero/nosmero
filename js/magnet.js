...
// ---------------------------------------------------------------------------
// 1. Magnets are stashed from the RAW note text, before parseContent() runs.
//    Two things would otherwise destroy them: escapeHtml() turns their `&`
//    separators into `&amp;`, and the URL pass would match the http(s):// URLs
//    sitting inside their own `tr=` and `ws=` parameters and rewrite them into
//    <a> tags in the middle of the URI.
//
// 2. The stash token is plain alphanumeric so it survives every later pass
//    untouched — URL_REGEX needs `http`, NOSTR_REGEX needs a bech32 prefix,
//    HASHTAG_REGEX needs `#`, and none of them match it. It deliberately
//    avoids NUL, which the HTML parser inside DOMPurify would turn into U+FFFD.
//
// Restoration happens at the end of parseContent, before the caller hands the
// result to sanitizeHtml(), so the card is sanitized like everything else.
//
// Note on the missing href: an <a href="magnet:..."> would be the natural
// affordance, but sanitizeHtml()'s ALLOWED_URI_REGEXP in utils.js only permits
// https?/nostr/monero/mailto, so the href would be silently stripped. Widening
// it would relax sanitizer policy for every surface in the app, so the card
// carries `data-magnet` and a delegated click handler instead — which is also
// what mobile's strict `script-src 'self'` requires (no inline handlers).
...
//
// The local escape below covers quotes as well as & < >. Magnets are stashed
// before the note body is escaped, so everything the card interpolates is raw
// relay input — an unescaped `"` would close the data-magnet attribute and
// inject markup.
function esc(value) {
    return String(value)
        .replace(/&/g, '&amp;')
...
async function notify(message, type) {
    try {
        ...
        Utils.toast?.(message, type, type === 'success' ? 1200 : 3000);
    } catch (e) {
