...
async function handleNavigation(event) {
    ...
    switch (tab) {
        ...
        case 'messages':
            ...
            break;
        case 'notifications':
            // Optimistically hide the badge while events stream in.
            // renderNotifications() will advance lastViewedNotificationTime to the
            // max timestamp of rendered items once they arrive, so late events
            // (still streaming after this click) properly count as unread on the
            // next refresh.
...
async function loadBookmarksPage() {
    ...
    try {
        ...
        // Addressable bookmarks (NIP-51 `a` tags) — currently kind 30023 articles,
        // forward-compatible for other addressable kinds.
        ...
        // --- Fetch kind-30023 articles in parallel ---
        let articleEvents = [];
        ...
        // Prefetch author profiles for all events before rendering
        const authors = [...new Set(allEvents.map(e => e.pubkey))];
        try { await Posts.fetchProfiles(authors); } catch {}

        // Render: kind-30023 → article card; everything else → renderSinglePost
        const Articles = await import('./articles.js');
        ...
        // Wire article-card clicks (the kind-1 posts already have their own handlers via render path)
    ...
    try {
        ...
        const sub = State.pool.subscribeMany(readRelays, [
            ...
            oneose(relay) {
                ...
                // Nostr kind 0 is a replaceable event — the canonical profile is the
                // single most-recent one. Relays often carry older copies alongside newer
                // ones; merging/scoring across them would resurrect fields the user removed.
                if (profileEvents.length > 0) {
                    console.log('Processing', profileEvents.length, 'profile events');

                    // Drop events older than our locally saved profile so stale relay
                    // copies can't overwrite a just-edited profile on reload.
                    ...
                    // Pick the single newest surviving event — do NOT merge fields from older ones.
    ...
    try {
        ...
        if (keyStorageStatus) {
            if (State.getPrivateKeyForSigning() === 'extension') {
                ...
                const loginMethod = localStorage.getItem('login-method');
                if (loginMethod === 'email_password') {
                    keyStorageStatus.textContent = '🔐 Password-protected (stored on server, session-only locally)';
                } else {
...
// Mark a Mute/Unmute button as in-flight so users get visible feedback
// while we wait on the relay publish round-trip.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // On initial page load the chip stays hidden — startApplication() will
