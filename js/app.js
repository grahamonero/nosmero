...
// v2.9.28 - Added nsec.app and Amber login options to auth UI
...
async function initializeApp() {
    ...
    // TEMPORARY: Clear NIP-05 cache to test verification
    Nip05.clearNip05Cache();
    console.log('🧹 Cleared NIP-05 cache');
    ...
    try {
        ...
        // show the thread page skeleton while the PIN modal is still up, instead
        // of flashing the home feed first.
        handleHashRouting();
        ...
        // Hash routing now runs earlier (right after Auth UI init) so the
        // thread page skeleton appears immediately rather than after feed render.
    ...
    page.innerHTML = `
        <div style="max-width: 800px; margin: 0 auto; padding: 16px;">
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                ...
                <h1 style="margin: 0; font-size: 20px; color: #fff;">★ Bookmarks</h1>
            </div>
...
async function displayNip05Verification(profile) {
    ...
    console.log('🔍 displayNip05Verification:', {
        hasContainer: !!nip05Container,
        nip05: profile.nip05,
        pubkey: profile.pubkey?.substring(0, 16)
    });

    if (!nip05Container || !profile.nip05) {
        console.log('⚠️ Skipping NIP-05:', !nip05Container ? 'no container' : 'no nip05');
        return;
    }

    // Safety check: don't verify lightning addresses as NIP-05
    if (profile.nip05 === profile.lud16 || profile.nip05 === profile.lud06) {
        console.log('⚠️ Skipping NIP-05: matches lightning address');
        return;
    }
    ...
    console.log('🔄 Starting NIP-05 verification for:', profile.nip05);

    // TEMPORARY: Clear cache before verifying logged-in user (eruda CORS workaround)
    if (profile.pubkey === State.publicKey) {
        console.log('🧹 Clearing NIP-05 cache for logged-in user');
        Nip05.clearNip05Cache();
    }
    ...
    try {
        ...
        console.log('✅ NIP-05 verification result:', verification);
    ...
    feed.innerHTML = `
        <div style="padding: 40px; text-align: center; max-width: 500px; margin: 0 auto;">
            ...
            <div style="display: flex; flex-direction: column; gap: 16px; margin-bottom: 30px;">
                ...
                <button onclick="showLoginWithNsecApp()"
                        style="padding: 16px 24px; border: none; border-radius: 12px; cursor: pointer; background: #333; color: #fff; font-weight: bold; font-size: 16px;">
                    🌐 Login with nsec.app
                </button>

                <button onclick="showLoginWithAmber()"
                        style="padding: 16px 24px; border: none; border-radius: 12px; cursor: pointer; background: #333; color: #fff; font-weight: bold; font-size: 16px;">
                    📱 Login with Amber
                </button>
            </div>
            ...
            <div style="font-size: 14px; color: #666; line-height: 1.4;">
                ...
                <p>Using nos2x, Alby, nsec.app, or Amber? Choose your preferred method.</p>
            </div>
        </div>
    `;
...
async function populateSettingsForm() {
    try {
        ...
        const personalizeScores = localStorage.getItem('personalizeScores') !== 'false';
        const shareData = localStorage.getItem('shareDataWithRelatr') === 'true';
...
async function populateRelayLists() {
    ...
    const renderRow = (relay, accentBg, removeFn, withAnnounce) => {
        ...
        const announceControl = withAnnounce
            ...
            : '';
        return `
            <div style="display: flex; flex-wrap: wrap; align-items: center; padding: 8px; background: ${accentBg}; border-radius: 6px; margin-bottom: 6px; gap: 8px;">
                <span style="color: #fff; font-family: monospace; font-size: 12px; flex: 1 1 100%; word-break: break-all;">${safeRelay}</span>
                ${announceControl}
                <button onclick="${removeFn}('${safeRelay}')"
                        style="background: #ff4444; border: none; border-radius: 4px; color: white; padding: 4px 8px; font-size: 12px; cursor: pointer; margin-left: auto;">
                    Remove
                </button>
            </div>
        `;
