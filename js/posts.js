...
async function loadTrendingFeedForAnonymous(forceRefresh = false) {
    ...
    try {
        ...
        try {
            if (window.NostrPaywall?.processPaywalledNotes && homeFeedList) {
                await window.NostrPaywall.processPaywalledNotes(homeFeedList);
            }
        } catch (error) {
            console.error('Error processing paywalled notes in trending feed:', error);
        }
...
async function renderCachedTrendingFeed(cache) {
    ...
    try {
        ...
        console.error('Error processing paywalled notes in cached trending feed:', error);
    }
...
async function renderCachedTrendingFeedForLoggedIn(cache) {
    ...
    try {
        if (window.NostrPaywall?.processPaywalledNotes) {
            await window.NostrPaywall.processPaywalledNotes(feed);
        }
    } catch (error) {
        console.error('Error processing paywalled notes in cached trending feed for logged-in user:', error);
    }
...
async function loadMoreTrendingPosts() {
    ...
    try {
        ...
        try {
            ...
            console.error('Error processing paywalled notes in load more trending posts:', error);
        }
    ...
    const postsHtml = postsToRender.map((post, index) => {
        ...
        return `
            <div class="post" data-post-id="${post.id}" data-pubkey="${post.pubkey}">
                ...
                ${isPaywalled ? `
                <div class="paywall-locked" data-note-id="${post.id}" onclick="openThreadView('${post.id}')" style="cursor: pointer;">
...
export async function likePost(postId) {
    ...
    try {
        if (isLiked) {
            ...
            const publishRelays = [...new Set([...myWriteRelays, ...authorInbox])];
            console.log('📤 Like publishing (NIP-65 outbox model):');
            console.log('  Your write relays:', myWriteRelays.length);
            console.log('  Author inbox relays:', authorInbox);
            console.log('  Combined publish relays:', publishRelays.length);
...
async function doQuickRepost() {
    ...
    const myWriteRelays = Relays.getWriteRelays();
    const publishRelays = [...new Set([...myWriteRelays, ...authorInbox])];
    console.log('📤 Repost publishing (NIP-65 outbox model):');
    console.log('  Your write relays:', myWriteRelays.length);
    console.log('  Author inbox relays:', authorInbox);
    console.log('  Combined publish relays:', publishRelays.length, publishRelays);

    const results = await State.pool.publish(publishRelays, signedEvent);
...
async function doQuoteRepost() {
    ...
    if (State.subaddressSettings?.perNote) {
        try {
            if (window.Wallet?.isWalletUnlocked?.()) {
                ...
                console.log(`[Posts] Quote repost using subaddress #${index}: ${address.slice(0, 10)}...`);
            } else {
                ...
                if (MoneroClient.isWalletUnlocked()) {
                    ...
                    moneroAddress = address;
                    console.log(`[Posts] Quote repost using subaddress #${index} (direct): ${address.slice(0, 10)}...`);
                }
            }
        } catch (e) {
            console.warn('[Posts] Could not generate subaddress for quote repost, using profile address:', e);
        }
    }
    ...
    const eventTemplate = {
        ...
        tags: [
            ['e', currentRepostPost.id, relayHint, 'mention'],
            ['p', currentRepostPost.pubkey, relayHint],
            ['q', currentRepostPost.id, relayHint], // Quote tag with relay hint
            ['client', 'nosmero']
        ],
        content: noteContent
    };
    ...
    const myWriteRelays = Relays.getWriteRelays();
    const publishRelays = [...new Set([...myWriteRelays, ...authorInbox])];
    console.log('📤 Quote repost publishing (NIP-65 outbox model):');
    console.log('  Your write relays:', myWriteRelays.length);
    console.log('  Author inbox relays:', authorInbox);
    console.log('  Combined publish relays:', publishRelays.length);
    await State.pool.publish(publishRelays, signedEvent);
    ...
    try {
        ...
        return `
            ...
            <div class="post" data-post-id="${post.id}" data-pubkey="${post.pubkey}" ${repostContext ? `data-repost-id="${repostContext.repostId}"` : ''}>
                ...
                ${isPaywalled ? `
                <div class="paywall-locked" data-note-id="${post.id}" ${context !== 'thread' ? `onclick="openThreadView('${post.id}')" style="cursor: pointer;"` : ''}>
                    <div class="paywall-preview">
                        <p>${Utils.escapeHtml(paywallMeta?.preview || 'Premium content')}</p>
                    </div>
                    <div class="paywall-overlay">
...
export async function sendPost() {
    ...
    try {
        ...
        // Reset paywall UI if it was used
...
function handleToolbarClick(e) {
    ...
    const composeArea = button.closest('.compose-area, #compose');
    if (!composeArea) return;
...
export async function publishNewPost() {
    ...
    const inputMoneroAddress = document.getElementById('modalMoneroAddress').value.trim();
    ...
    try {
        ...
        // Determine Monero address - use subaddress if enabled and wallet unlocked
        let moneroAddress = inputMoneroAddress || State.userMoneroAddress;
        if (State.subaddressSettings?.perNote) {
            try {
                if (window.Wallet?.isWalletUnlocked?.()) {
                    const { address, index } = await window.Wallet.getNextSubaddress();
                    moneroAddress = address;
                    console.log(`[Posts] Modal post using subaddress #${index}: ${address.slice(0, 10)}...`);
                } else {
                    const MoneroClient = await import('./wallet/monero-client.js');
                    if (MoneroClient.isWalletUnlocked()) {
                        const { address, index } = await MoneroClient.getNextSubaddress();
                        moneroAddress = address;
                        console.log(`[Posts] Modal post using subaddress #${index} (direct): ${address.slice(0, 10)}...`);
                    }
                }
            } catch (e) {
                console.warn('[Posts] Could not generate subaddress for modal post, using profile address:', e);
            }
        }
...
export async function showDisclosedTipDetails(postId, event) {
    ...
    let disclosedTips = disclosedTipsCache[postId];

    // If cache is empty or has no tips, fetch fresh data from relay
    if (!disclosedTips || disclosedTips.tips.length === 0) {
        console.log('💰 Cache empty for', postId, '- fetching fresh tip data...');
        const freshData = await fetchDisclosedTips([postId]);
        if (freshData[postId]) {
            disclosedTipsCache[postId] = freshData[postId];
            disclosedTips = freshData[postId];
        }
    }
...
export async function loadTipActivityFeed() {
    try {
        console.log('🔄 loadTipActivityFeed starting...');
        ...
        if (feed) feed.innerHTML = '<div class="loading">Fetching network stats...</div>';
        await fetchWidgetNetworkStats();
        console.log('✅ Network stats fetched');

        if (State.publicKey) {
            console.log('📊 Fetching personal stats...');
            if (feed) feed.innerHTML = '<div class="loading">Fetching your received tips...</div>';
            await fetchWidgetPersonalStats();
            console.log('✅ Personal stats fetched');

            console.log('📊 Fetching sent stats...');
            if (feed) feed.innerHTML = '<div class="loading">Fetching your sent tips...</div>';
            await fetchWidgetSentStats();
            console.log('✅ Sent stats fetched');
        }

        // Render the widget content in the feed area
        console.log('🎨 Rendering tip activity widget...');
        if (feed) feed.innerHTML = '<div class="loading">Rendering tip activity...</div>';
        renderTipActivityWidget();
        console.log('✅ Tip activity feed loaded!');
