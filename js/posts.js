...
// Mute list handling moved to js/lists.js (NIP-51 standard kind 10000).
// These exports remain as thin delegates so existing callers keep working.
...
export async function muteUser(pubkey) {
    ...
    const Lists = await import('./lists.js');
    // Let errors propagate so the caller's .catch can surface them to the user.
...
async function renderHomeFeedResults() {
    ...
    try {
        ...
        // Module not loaded yet — fall back to legacy pubkey-only filter
...
async function fetchMissingProfilesViaNIP65(missingPubkeys) {
    ...
    try {
        ...
        // window.location.hash === '#home', but neither desktop nor mobile
        // uses that hash for the home feed. Use State.currentPage so the
        // re-render fires regardless of platform / route.
...
export async function renderSinglePost(post, context = 'feed', engagementData = null, parentPostsMap = null, repostContext = null) {
    try {
        ...
        // Module may not be loaded yet on first paint — that's fine, just skip.
        const Lists = _ListsRef || await _loadLists();
        if (Lists?.isMuted) {
            // For reposts we also need to check the reposter via repostContext
...
export function formatText(textarea, format) {
    ...
    switch (format) {
        ...
        case 'mention': {
            const input = prompt('Paste npub or nprofile:');
            if (!input) return;
...
function showIPFSEmbedModal(textarea, selectedText, start, end) {
    ...
    let quotaUsed = 0, quotaTotal = 524288000; // safe defaults until refresh
    ...
    // ---- Close: handle paste-tab submit (the only path through the form),
    // tear down all listeners, reset state.
    const closeHandler = () => {
    ...
    function cleanupAndClose() {
        modal.close(); // triggers closeHandler — paste returnValue empty so no insert
    }
...
export function initComposeToolbar() {
    ...
    document.addEventListener('click', (e) => {
        ...
        if (e.target.classList.contains('emoji')) {
            ...
            const composeArea = picker?.closest('.compose-area, .right-panel-compose, #compose');
            const textarea = composeArea?.querySelector('.compose-textarea');
            if (textarea && emoji) {
